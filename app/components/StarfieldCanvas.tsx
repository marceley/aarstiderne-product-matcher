...
interface Star extends Vec3 { phase: number; }
...
export default function StarfieldCanvas({
  ...
  const project = (p: Vec3, w: number, h: number, fov = 500): { x: number; y: number; scale: number } => {
  ...
  useEffect(() => {
    ...
    const setSize = (): void => {
      const dpr = Math.max(1, Math.min(2, (window.devicePixelRatio || 1) as number));
    ...
    const draw = (): void => {
      ...
      targetRef.current.x = Math.max(-Math.PI/3, Math.min(Math.PI/3, targetRef.current.x));
      targetRef.current.y = Math.max(-Math.PI, Math.min(Math.PI, targetRef.current.y));

      anglesRef.current.x += (targetRef.current.x - anglesRef.current.x) * inertia;
      anglesRef.current.y += (targetRef.current.y - anglesRef.current.y) * inertia;
      ...
      for (let i = 0; i < baseRef.current.length; i++) {
        ...
        const twinkle = 1 - twinkleIntensity * (0.5 + 0.5 * Math.sin(twinklePhase.current + star.phase));

        // Fade farther stars to create depth
        const distanceFade = 1 - Math.min(1, ((p.z + 500) / 1000) * depthFade);
        const alpha = Math.max(0, Math.min(1, brightness * twinkle * distanceFade));
    ...
    const onScroll = (e: WheelEvent): void => {
      ...
      scrollVelocityRef.current.x = Math.max(-maxVelocity, Math.min(maxVelocity, scrollVelocityRef.current.x));
      scrollVelocityRef.current.y = Math.max(-maxVelocity, Math.min(maxVelocity, scrollVelocityRef.current.y));
    };
  ...
  }, [count, inertia, dotSize, brightness, sensitivity, twinkleSpeed, twinkleIntensity, depthFade]);
